<template>
  <div>
    ...
    <VCard title="Update Data of this Event">
      <VAlert
...
export default {
  ...
  methods: {
    async fetchData() {
      this.loader = true;
      await http
        .get("/event/show/" + this.paramsId)
        .then((res) => {
          if (res.data.success) {
            const resData = res.data.data;
            this.insertData.time = resData.time;
            ...
            this.insertData.phone = resData.phone;
    ...
    async updateData() {
      ...
      if (checkValidation.valid) {
        ...
        this.loader = true;
        http
          .post("event/update/" + this.paramsId, formData)
          .then((res) => {
            if (res.data.success) {
              this.fetchData();
