<template>
  <div>
    ...
    <VCard title="Update Data of this Event Speaker">
      <VAlert
      ...
      <VForm ref="formSubmit">
        <VCardText>
          <VRow>
            <VCol cols="12" md="4">
              <AppSelect
                v-model="insertData.event_id"
                :items="data_fetch_events"
  ...
  methods: {
    fetch_events() {
      http
        ...
        .then((res) => {
          if (res.data.success) {
            this.data_fetch_events = res.data.data;
          }
        })
        .catch((e) => {
    ...
    async fetchData() {
      this.loader = true;
      await http
        .get("/event-speaker/show/" + this.paramsId)
        .then((res) => {
          if (res.data.success) {
            const resData = res.data.data;
            this.insertData.event_id = resData.event_id;
            this.insertData.name = resData.name;
            this.insertData.designation = resData.designation;
            ...
            this.insertData.linkedin_link = resData.linkedin_link;
    ...
    async updateData() {
      ...
      if (checkValidation.valid) {
        ...
        this.loader = true;
        http
          .post("event-speaker/update/" + this.paramsId, formData)
          .then((res) => {
            if (res.data.success) {
              this.fetchData();
              this.$toast.success(res.data.message);
              this.$router.push({
                path: "/eventSpeaker/list/",
              });
